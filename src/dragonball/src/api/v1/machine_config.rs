...
/// We only support 254 vcpus for now. Mostly because we have set all vcpu related metrics as u8
...
pub enum VmConfigError {
    ...
    #[error("the vCPU number shouldn't large than 254")]
    ...
    #[error("the hotplug memory size (MiB) is invalid, must be multiple of 64MB")]
    InvalidHotplugMemorySize(usize),

    /// The reserved memory size is invalid. The memory can only be an unsigned integer.
    #[error("the reserved memory size (MiB) is invalid, must be multiple of 64MB")]
    InvalidReservedMemorySize(usize),
